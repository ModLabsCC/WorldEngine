package com.liamxsage.worldengine.presets;
...
/*		Grad(double x, double y, double z, double w)
	...
	public static void generateSimplexNoise(int startX, int startY, int width, int height, float frequency, float[][] storage){
		for(int i = 0, x = startX; x < startX + width; i++, x++){
			for(int j = 0, y = startY; y < startY + height; j++, y++){
				storage[i][j] = (float) noise(x * frequency,y * frequency);
